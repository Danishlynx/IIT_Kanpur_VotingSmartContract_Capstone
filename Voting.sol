...
contract Voting {
    ...
    function addCandidate(string memory _name, string memory _proposal, address _owner) public onlyAdmin {
    candidates[candidateIdCounter] = Candidate(_name, _proposal, 0);
    candidateIdCounter++;
    }

    function addVoter(address _voterAddress, address _owner, string memory _name) public onlyAdmin {
    ...
    function startElection(address ownerAddress) public onlyAdmin {
    require(ownerAddress == admin, "Only the contract owner can start the election.");
    ...
    function endElection(address ownerAddress) public {
    require(ownerAddress == admin, "Only the contract owner can end the election.");
